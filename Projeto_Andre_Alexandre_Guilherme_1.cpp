...
void cadastrarNovaPessoa(bool &continuar, int &linhas, pessoa cadastros[]) {
    ...
    for (size_t i = 0; i < strlen(novaPessoa.tipoSanguineo); ++i) {
    ...
    // Open the existing binary file in both read and write modes
    ...
    // Move to the end of the file to append the new entry
    binario.seekg(0, ios::end);
    
    // Write the new entry to the binary files
...
void alterarpessoa(pessoa nomeProcurado[], int linhas, bool &continuar) {
    ...
    if (encontrar) {
        ...
        // Abrindo o arquivo
        ofstream binario("dados_hemograma.bin", ios::binary);
        if (!binario) {
            cout << "Erro ao abrir o arquivo binário" << endl;
            return;
        }
